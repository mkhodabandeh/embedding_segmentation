...
machine = 'cs-vml-24'
parameters = {
        # '-v':['samba_kids', 'panda_cub','ballet' ],
        '-v':['salsa', 'palm_tree', 'harley_davidson', 'jugle_cat', 'airplane'],
        # '-b': [128],
        # '-a': [12],
        # '-A': [4],
        # '-S': [400],
...
def dfs(i, params_list, cmd, processes):
    if i == len(params_list):
        ...
        processes.append('python {0}/exec_scripts.py {1} -c {2} -f -s > {3}/exp_{2}.log 2>&1'.format(proj_path,params_str, comment_str, log_path))
