...
def do(name):
    ...
    for i,gt_num in enumerate(gts):
        ...
        os.system('cp -r /cs/vml2/mkhodaba/cvpr16/VSB100/fake /cs/vml2/mkhodaba/cvpr16/VSB100/VideoProcessingTemp/{}'.format(name+str(i)))
        s = max(start, gt_num-10)
        e = min(num_frames+start, gt_num+10)
        print i, gt_num, ' frames:', s, '-',e
        for j in xrange(1,5):
            os.system('mkdir -p /cs/vml2/mkhodaba/cvpr16/VSB100/VideoProcessingTemp/{}/gtimages/gt_{}'.format(name+str(i), j))
            os.system('cp '+\
                    ' /cs/vml2/mkhodaba/cvpr16/VSB100/VideoProcessingTemp/{0}/gtimages/gt_{1}/gtimages_{2:08d}.png'.format(name, j, gt_num) +\
                    ' /cs/vml2/mkhodaba/cvpr16/VSB100/VideoProcessingTemp/{0}/gtimages/gt_{1}/gtimages_{2:08d}.png'.format(name+str(i), j, gt_first))
...
if __name__=='__main__':
    ...
    main(names[start:min(len(names), start+20)])
